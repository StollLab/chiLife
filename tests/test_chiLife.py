...
def test_repack():
    ...
    traj2, deltaE2 = chilife.repack(
        protein, SL, repetitions=10, off_rotamer=True, repack_radius=10
    )
...
def test_create_library():
    ...
    np.testing.assert_allclose(
        SL.spin_centers, np.array([[-24.444479, 15.084169, 14.754088]])
    )
    ...
    np.testing.assert_almost_equal(
        SL.coords[0, 5:], struct.atoms.positions[5:], decimal=3
    )
...
def test_create_arb_rotlib():
    ...
    with pytest.raises(RuntimeError) as err:
        ...
        errstr = (
            "RuntimeError: The residue/rotlib you have selected TUM, does not share a compatible backbone with the "
...
def test_single_chain_error():
    with pytest.raises(RuntimeError):
        chilife.create_dlibrary(
            libname="___",
            pdb="test_data/chain_broken_dlabel.pdb",
            sites=(15, 17),
            dihedral_atoms=[
                [["N", "CA", "C13", "C5"], ["CA", "C13", "C5", "C6"]],
                [["N", "CA", "C12", "C2"], ["CA", "C12", "C2", "C3"]],
            ],
            spin_atoms="Cu1",
        )
...
def test_use_local_library():
    weights = np.loadtxt("test_data/R1M.weights")
    dihedral_names = np.loadtxt("test_data/R1M.chi", dtype="U").tolist()
    spin_atoms = ["N1", "O1"]
    chilife.create_library(
        "RXL",
        "test_data/R1M.pdb",
...
def test_add_new_default_library():
    ...
    oweights = np.loadtxt("test_data/R1M.weights")
    dihedral_names = np.loadtxt("test_data/R1M.chi", dtype="U").tolist()
    spin_atoms = ["N1", "O1"]
    chilife.create_library(
        "RXL",
...
def test_repack_with_custom_rotlib():
    ...
    XYZ41 = chilife.SpinLabel(
        "XYZ", 41, omp, rotlib="test_data/usr_rtlb/XYZ_rotlib.npz"
    )

    traj, dE = chilife.repack(
        omp, XYZ41, repetitions=50, temp=298, off_rotamer=True, repack_radius=10
    )
    T4L131R9R = chilife.SpinLabel.from_trajectory(
        traj, 41, burn_in=5, spin_atoms=XYZ41.spin_atoms
    )
