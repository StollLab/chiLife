...
class RotamerLibrary:
    ...
    def __init__(self, res, site=1, protein=None, chain=None, **kwargs):
        ...
        self.atoms = [ProEPR.Atom(name, atype, idx, self.res, self.site, coord) for idx, (coord, atype, name) in
                      enumerate(zip(self.coords[0], self.atom_types, self.atom_names))]

        self.mx, self.ori = ProEPR.global_mx(*np.squeeze(self.backbone))
        self.ic_ori, self.ic_mx = None, None
    ...
    def from_trajectory(cls, traj, site, energy=None, burn_in=100,  **kwargs):
        ...
        for i in range(len(prelib.weights)):
            dihedrals.append([ProEPR.get_dihedral(coords[i][mask]) for mask in masks])
    ...
    def __deepcopy__(self, memodict={}):
        new_copy = ProEPR.RotamerLibrary(self.res, self.site, self.chain)
    ...
    def _to_site(self, site_pos=None):
        ...
        """
        if site_pos is None:
            N, CA, C = ProEPR.parse_backbone(self, kind='global')
        else:
            N, CA, C = site_pos

        mx, ori = ProEPR.global_mx(N, CA, C, method=self.superimposition_method)

        if self.superimposition_method not in {'fit', 'rosetta'}:
            N, CA, C = ProEPR.parse_backbone(self, kind='local')
            old_ori, ori_mx = ProEPR.local_mx(N, CA, C, method=self.superimposition_method)
            self.coords -= old_ori
            mx = ori_mx @ mx
    ...
    def _off_rotamer_sample(self, idx, off_rotamer, **kwargs):
        ...
        if self.ic_ori is None:
            self.ic_ori, self.ic_mx = ProEPR.local_mx(*np.squeeze(coords[self.backbone_idx]))
    ...
    def minimize(self):
        ...
        rmin_ij, eps_ij = ProEPR.get_lj_params(self.rmin2, self.eps,
                                               self.protein.atoms.types[protein_clash_idx], 1, forgive=1)

        a, b = [list(x) for x in zip(*self.non_bonded)]
        a_eps = ProEPR.get_lj_eps(self.atom_types[a])
        a_radii = ProEPR.get_lj_rmin(self.atom_types[a])
        b_eps = ProEPR.get_lj_eps(self.atom_types[b])
        b_radii = ProEPR.get_lj_rmin(self.atom_types[b])
        ...
        for i, IC in enumerate(self.internal_coords):
            ...
            self.weights[i] = self.weights[i] * np.exp(-xopt.fun / (ProEPR.GAS_CONST*298))
    ...
    def evaluate_clashes(self, environment, environment_tree=None, ignore_idx=None, temp=298):
        ...
        probabilities = ProEPR.evaluate_clashes(ori=self.clash_ori, label_library=self.coords[:, self.side_chain_idx],
        ...
        self.weights, self.partition = ProEPR.reweight_rotamers(probabilities, self.weights, return_partition=True)
    ...
    def save_pdb(self, name=None):
        if name is None:
            name = self.name
        ProEPR.save_rotlib(name, self.atoms, self.coords)
    ...
    def get_lib(self):
        ...
        Phi = None if PhiSel is None else ProEPR.get_dihedral(PhiSel.positions)
        Psi = None if PsiSel is None else ProEPR.get_dihedral(PsiSel.positions)
