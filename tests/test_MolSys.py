...
def test_parse_paren4():
    P = parse_paren(
        "resname LYS ARG PRO and (name CA or (name CB and resname PRO)) or resnum 5"
    )
    assert len(P) == 3
...
def test_select_complex():
    ...
    ca_or_c_pro = (prot.names == "CA") + (
        (prot.atypes == "C") * (prot.resnames == "PRO")
    )
...
def test_set_residue_bs():
    prot2 = MolSys.from_pdb("test_data/1omp_H.pdb")
    print(prot2.bs.max())
    asel = prot2.select_atoms("resname ARG").residues[0]
    asel.bs = 2
    mask = asel.mask
    assert np.all(asel.bs == 2)
    assert np.all(prot2.bs[mask] == 2)
    print(prot2.bs.max())
