...
class dRotamerLibrary(RotamerLibrary):
    ...
    def get_lib(self):
        ...
        self.SL2 = chiLife.SpinLabel(self.label + f'ip{self.increment}', self.site2, self.protein, self.chain,
                                     **self.kwargs)
    ...
    def _minimize(self):
        ...
        def objective(dihedrals, ic1, ic2, opt):
            coords1 = ic1.set_dihedral(dihedrals[:len(self.SL1.dihedral_atoms)], 1,
                                       self.SL1.dihedral_atoms).to_cartesian()
            coords2 = ic2.set_dihedral(dihedrals[-len(self.SL2.dihedral_atoms):], 1,
                                       self.SL2.dihedral_atoms).to_cartesian()
