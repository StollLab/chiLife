...
def fit_superimposition(N, CA, C):
    ...
    if len(N) != len(CA) != len(C) != 2:
        raise ValueError("fit_superimposition takes 3 sets of 2 coordinates. The first coordinate should be from the "
                         "mobile residue and the second coordinate should be from the target residue.")

    raise NotImplementedError('Superimposition by minimizing RMSD of the backbone coordinates it not yet implemented')
...
def parse_backbone(rotamer_library, kind):
    ...
    if method in {'fit'}:
        ...
        N2, CA2, C2 = rotamer_library.protein.select_atoms(f'segid {rotamer_library.chain} and '
                                                           f'resnum {rotamer_library.site} '
                                                           f'and name N CA C and not altloc B').positions
        return np.array([[N1, N2], [CA1, CA2], [C1, C2]])
    elif kind == 'local':
        return rotamer_library.backbone
    elif kind == 'global':
        return rotamer_library.protein.select_atoms(f'segid {rotamer_library.chain} and '
                                                    f'resnum {rotamer_library.site} '
                                                    f'and name N CA C and not altloc B').positions
