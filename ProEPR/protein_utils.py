...
class ProteinIC:
    ...
    def save_pdb(self, filename: str, mode='w'):
        ...
        if 'a' in mode:
            with open(str(filename), mode, newline='\n') as f:
                f.write("MODEL\n")
            ProEPR.save_pdb(filename, self.atoms, self.to_cartesian(), mode=mode)
            with open(str(filename), mode, newline='\n') as f:
                f.write("ENDMDL\n")
        else:
            ProEPR.save_pdb(filename, self.atoms, self.to_cartesian(), mode=mode)
    ...
    def collect_dih_list(self) -> list:
        ...
        for resi in self.ICs[1]:
            ...
            res_dihs += ProEPR.dihedral_defs.get(resname, [])
...
def get_internal_coords(mol: Union[MDAnalysis.Universe, MDAnalysis.AtomGroup],
    ...
    if resname is not None:
        protein = U.select_atoms(f'(protein or resname {" ".join(list(ProEPR.SUPPORTED_RESIDUES) + [resname])})'
                                 f' and not altloc B')
    else:
        protein = U.select_atoms(f'(protein or resname {" ".join(list(ProEPR.SUPPORTED_RESIDUES))}) and not altloc B')
    ...
    for atom in protein.atoms:
        ...
        if atom.index == 0 or atom.resid - U.atoms[atom.index - 1].resid not in (0, 1):
            segid += 1
            offset = atom.index
            mx, ori = ProEPR.ic_mx(*U.atoms[offset:offset+3].positions)
...
def save_pdb(name: str, atoms: ArrayLike, coords: ArrayLike, mode: str = 'w') -> None:
    ...
    :param atoms: list, tuple
        List of ProEPR.Atom objects to be saved
...
def get_missing_residues(protein: Union[MDAnalysis.Universe, MDAnalysis.AtomGroup],
    ...
    for res in protein.residues:
        # Only consider supported residues because otherwise ProEPR wouldn't know what's missing
        if res.resname not in ProEPR.SUPPORTED_RESIDUES or res.resnum in ignore or res.resname in ['ALA', 'GLY']:
            continue
        ...
        if len(heavy_atoms) != cache.get(res.resname, len(RotamerLibrary(res.resname).atom_names)):
            missing_residues.append(RotamerLibrary(res.resname, res.resnum, protein=protein, use_H=use_H))
...
def mutate(protein: MDAnalysis.Universe,
    ...
    :param rotlibs: ProEPR.RotamerLibrary, ProEPR.SpinLabel
    ...
    protein = protein.select_atoms(f'(protein or resname {" ".join(ProEPR.SUPPORTED_RESIDUES)}) and not altloc B')
