...
def test_from_prot():
    ...
    np.testing.assert_almost_equal(
        ubqIC.trajectory.coordinate_array[:, ubqIC.non_nan_idxs],
...
def test_from_prot_traj():
    ...
    np.testing.assert_equal(
        mbpIC.z_matrix_idxs[mbpIC.non_nan_idxs], zmat_idx_ans[mbpIC.non_nan_idxs]
    )
    np.testing.assert_almost_equal(
        mbpIC.trajectory.coordinate_array[:, mbpIC.non_nan_idxs],
...
def test_get_internal_coordinates(pdbid):
    protein = mda.Universe(f"test_data/{pdbid}.pdb", in_memory=True).select_atoms(
        "protein and not altloc B"
    )
    ...
    protein = xl.MolSys.from_pdb(f"test_data/{pdbid}.pdb").select_atoms(
        "protein and not altloc B"
    )
    ICs = xl.MolSysIC.from_atoms(protein, ignore_water=False)
    np.testing.assert_almost_equal(ICs.coords, protein.atoms.positions, decimal=4)
...
def test_copy():
    ...
    assert np.all(
        ubqIC.trajectory.coordinate_array == my_copy.trajectory.coordinate_array
    )
...
def test_ic_pref_dihe():
    ...
    np.testing.assert_almost_equal(
        sister_dihe_atom_coords, [2.1495337, -3.4064763, -0.9388056], decimal=6
    )
...
def test_sample():
    ...
    np.testing.assert_almost_equal(
        ICX.protein.trajectory.coordinate_array, ans, decimal=6
    )
