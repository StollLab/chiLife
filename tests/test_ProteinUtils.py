...
exou = mda.Universe("test_data/m3tu3.pdb", in_memory=True)
...
def test_mutate6():
    ...
    np.testing.assert_allclose(
        labeled_protein.select_atoms(SL.selstr).positions, SL.coords[10]
    )
...
def test_get_dihedrals():
    ...
    ans = [
        chilife.get_dihedral([N1, CA, C, N2])
        for N1, CA, C, N2 in zip(N1s, CAs, Cs, N2s)
    ]
...
def test_make_peptide():
    pep = chilife.make_peptide(
        "ACDEF[R1M]GHIKL<COC1=CC=C(C=C1)CC(C(=O)O)N>MNPQR[R1C]STVWY"
    )
...
def test_make_strand():
    ...
    pep = chilife.make_peptide(
        "ACDEF[R1M]GHIKL<COC1=CC=C(C=C1)CC(C(=O)O)N>MNPQR[R1C]STVWY", phi, psi, omega
    )
...
def test_make_pep_w_cap3():
    ...
    np.testing.assert_almost_equal(pep.positions, ans, decimal=2)
...
def test_smiles2residue2():
    m1 = chilife.smiles2residue(
        "C1C[C@H]2[C@@H]([C@@H]2C1)C(=O)C[C@H]([NH3+])C(=O)[O-]", randomSeed=0
    )
    test_mask = np.argsort(m1.names)
    test = m1.positions[test_mask]
...
def test_append_cap():
    ...
    nhh_coords = np.array(
        [
            [21.63728595, 20.70101107, 18.63843541],
...
def test_add_cap():
    ...
    assert (chilife.RL_DIR / 'cap_pdbs/XXX.pdb').exists()
    assert (chilife.RL_DIR / 'cap_pdbs/ZZZ.pdb').exists()

    os.remove(chilife.RL_DIR / 'cap_pdbs/XXX.pdb')
    os.remove(chilife.RL_DIR / 'cap_pdbs/ZZZ.pdb')


def test_guess_chain_unit():
    chain = chilife.guess_chain(exou.select_atoms('protein'), 456)
    assert chain == 'B'



def test_new_chain():
    chain_id = chilife.new_chain(ubq)
    assert chain_id == 'B'
