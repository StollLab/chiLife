...
def get_lj_params(label_lj_rmin2, label_lj_eps, environment_atypes, lib_len, forgive):
    ...
    :return:
    """
    protein_lj_radii = ProEPR.get_lj_rmin(environment_atypes)
    protein_lj_eps = ProEPR.get_lj_eps(environment_atypes)
    join_rmin = ProEPR.get_lj_rmin('join_protocol')[()]
    join_eps = ProEPR.get_lj_eps('join_protocol')[()]
...
def set_lj_params(forcefield):
    ProEPR.using_lj_param = forcefield
    rmin_func, eps_func = lj_params[forcefield]
    ProEPR.get_lj_rmin = np.vectorize(rmin_func.__getitem__)
    ProEPR.get_lj_eps = np.vectorize(eps_func.__getitem__)
